...
export default function ItemDetailClient({ meal }: ItemDetailClientProps) {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 sm:p-8">
            ...
            <motion.h1
                className="text-3xl md:text-4xl font-bold mb-6 text-teal-300"
            ...
            <div className="flex gap-4 mb-6">
                ...
                {meal.strYoutube && (
                    <motion.a
                        ...
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className="bg-red-500 hover:bg-red-500 text-white px-8 py-2 rounded-md font-semibold transition-colors">
                        ▶ Watch on YouTube
                    </motion.a>
                )}
            </div>
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {ingredients.map((ing, i) => (
                        <motion.li
                            ...
                            {ing.ingredient} -{" "}
                            <span className="text-gray-200">{ing.measure}</span>
                        </motion.li>
                    ))}
                </ul>
            </motion.div>
